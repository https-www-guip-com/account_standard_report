...
D_LEDGER = {'general': {'name': _('Libro mayor'),
            ...
            'partner': {'name': _('Libro mayor de socios'),
                        'group_by': 'partner_id',
                        'model': 'res.partner',
                        'short': 'name',
                        },
            'journal': {'name': _('Libro mayor de diarios'),
                        'group_by': 'journal_id',
            ...
            'aged': {'name': _('Balance envejecido'),
                     'group_by': 'partner_id',
                     'model': 'res.partner',
                     'short': 'name',
                     },
...
class AccountStandardLedgerLines(models.TransientModel):
    ...
    line_type = fields.Selection([('0_init', 'Incial'), ('1_init_line', 'Linea Inicial'),
                                  ('2_line', 'Línea'), ('3_compact', 'Compactado'), ('4_total', 'Total'),
                                  ('5_super_total', 'Super Total')], string='Tipo')
    view_type = fields.Selection([('init', 'Inicio'), ('normal', 'Normal'), ('total', 'Total')])
    journal_id = fields.Many2one('account.journal', 'Diario')
    partner_id = fields.Many2one('res.partner', 'Socios')
    group_by_key = fields.Char()
    move_id = fields.Many2one('account.move', 'Entradas')
    ...
    full_reconcile_id = fields.Many2one('account.full.reconcile', 'Coincidencia.')
    reconciled = fields.Boolean('Reconciliado')
    report_object_id = fields.Many2one('account.report.standard.ledger.report.object')

    current = fields.Monetary(default=0.0, currency_field='company_currency_id', string='No debido')
    age_30_days = fields.Monetary(default=0.0, currency_field='company_currency_id', string='30 dias')
    age_60_days = fields.Monetary(default=0.0, currency_field='company_currency_id', string='60 dias')
    age_90_days = fields.Monetary(default=0.0, currency_field='company_currency_id', string='90 dias')
    age_120_days = fields.Monetary(default=0.0, currency_field='company_currency_id', string='120 dias')
    older = fields.Float(default=0.0, digits=dp.get_precision('Account'), string='Más viejo')

    amount_currency = fields.Monetary(default=0.0, currency_field='currency_id', string='Monto de dinero')
...
class AccountStandardLedger(models.TransientModel):
    ...
    ledger_type = fields.Selection(
        [('general', 'Libro mayor'),
         ...
         ('journal', 'Libro mayor de diarios'),
    ...
    summary = fields.Boolean('Balance de prueba', default=False,
                             help=' * Check : generate a trial balance.\n'
                             ' * Uncheck : detail report.\n')
    amount_currency = fields.Boolean('Con moneda', help='It adds the currency column on report if the '
    ...
    analytic_account_select_ids = fields.Many2many(comodel_name='account.analytic.account', string='Cuentas Analiticas')
    ...
    result_selection = fields.Selection([('customer', 'Clientes'),
                                         ...
                                         ], string="Seleccione Socios", required=True, default='supplier')
    ...
    partner_ids = fields.Many2many('res.partner', string="Socios en informe", relation='table_standard_report_partner')
    report_type = fields.Selection([('account', 'Cuentas'), ('partner', 'Socios'), ('journal', 'Diarios'),
                                    ('analytic', 'Analitico')], string='Tipo  reporte')
